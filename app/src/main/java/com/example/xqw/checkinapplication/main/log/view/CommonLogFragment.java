 ...
 */
public class CommonLogFragment extends Fragment {
    @Bind(R.id.list_log)
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        listLog.setAdapter(new CommonAdapter<Map>(getActivity(),R.layout.card_view_common,getSomeData()) {
            @Override
            public void convert(ViewHolder holder, Map map) {
                TextView logState=holder.getView(R.id.state);
                final String[] colors = getContext().getResources().getStringArray(R.array.default_preview);
                holder.setText(R.id.text2,map.get("logContent").toString());
                holder.setText(R.id.state,map.get("logState").toString());
                holder.setText(R.id.text1,map.get("logTime").toString());
